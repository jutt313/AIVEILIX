...
> This document describes the complete database structure for Aiveilix. It covers all tables in PostgreSQL, collections in Qdrant, and keys in Valkey — with column definitions, relationships, and examples for every table.
...
| **Valkey** | Cache, sessions, and job queues |
...
| `web_search_mode` | ENUM | `smart`, `bucket_only`, `always_search` |
| `follow_up_mode` | ENUM | `all_at_once`, `one_by_one` |
| `created_at` | TIMESTAMP | Creation time |
...
| `parent_message_id` | UUID | Optional self-reference for reply chains |
...
Stores chunked conversation memory for thread-level RAG. The actual vector is stored in Qdrant, referenced by the same `id`.
...
| `user_id` | UUID | Foreign key — `users.id` |
| `role` | ENUM | `user`, `assistant` |
| `content` | TEXT | Chunk text used for conversation retrieval |
| `chunk_index` | INTEGER | Order of the chunk within the message |
| `token_count` | INTEGER | Number of tokens in this chunk |
| `status` | ENUM | `pending`, `embedded`, `failed` |
| `created_at` | TIMESTAMP | Creation time |
...
### `conversation_chunks` Collection

| Field | Type | Description |
|---|---|---|
| `id` | UUID | Same as `conversation_chunks.id` in PostgreSQL |
| `vector` | float[] | Embedding vector for conversation memory |
| `payload.conversation_id` | UUID | Reference to `conversations.id` |
| `payload.message_id` | UUID | Reference to `messages.id` |
| `payload.bucket_id` | UUID | Reference to `buckets.id` |
| `payload.user_id` | UUID | Reference to `users.id` |
| `payload.role` | ENUM | `user`, `assistant` |
| `payload.content` | TEXT | Conversation chunk text |
| `payload.chunk_index` | INTEGER | Order of the chunk within the message |
| `payload.status` | ENUM | `active`, `deprecated` |
...
User sends query to bucket
        |
        v
Valkey checks session token (auth)
        |
        v
Qdrant hybrid search on:
- text_chunks (bucket documents)
- conversation_chunks (current thread memory)
        |
        v
...
Conversation memory saved to PostgreSQL + Qdrant
Usage updated in Valkey cache + PostgreSQL (usage_tracking)
        |
        v
