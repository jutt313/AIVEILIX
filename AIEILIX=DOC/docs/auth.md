...
User submits email + password
        |
        v
Check failed_attempts in Valkey
...
Verify password against bcrypt hash
        ...
        v
If wrong password:
  Increment failed_attempts counter in Valkey
  Return clean error message
        |
        v
If correct:
  Reset failed_attempts counter in Valkey
  Issue JWT access token (24 hours)
  Issue refresh token (30 days) → save in Valkey
  Return tokens to frontend
```
...
|---|---|
| Expiry | 30 days |
| Storage | Valkey + HttpOnly cookie |
...
POST /auth/refresh
        |
        v
FastAPI validates refresh token in Valkey
        |
        v
...
POST /auth/logout
        |
        v
Backend adds current JWT to Valkey blacklist
Key: blacklist:{token_jti}
TTL: matches remaining token expiry time
        |
        v
Backend deletes refresh token from Valkey
        |
        v
...
**Why blacklist in Valkey?**
JWT tokens cannot be invalidated after they are issued. Adding the token ID (`jti`) to a Valkey blacklist ensures the token is rejected on every subsequent request, even before it expires.
...
If exists:
  ...
  Save to Valkey with 1 hour TTL:
...
User clicks link → enters new password
        |
        v
Backend validates token from Valkey
        |
        v
Hash new password with bcrypt
Update password_hash in PostgreSQL
Delete reset token from Valkey
        |
        v
...
JWT access token (24hrs) + Refresh token (30 days)
        ...
        v
Every API request:
  Valkey checks JWT not blacklisted
